...
if (isset($_GET['delete'])) {
    // Check if location is assigned to any asset
    $stmt = $pdo->prepare('SELECT COUNT(*) FROM assets WHERE location_id=?');
    $stmt->execute([$_GET['delete']]);
    $in_use = $stmt->fetchColumn();
    if ($in_use > 0) {
        $delete_error = 'Cannot delete: This location is assigned to one or more assets.';
    } else {
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
  ...
  <h2 class="text-xl font-bold tracking-tight text-gray-900">Locations</h2>
    <button class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold transition" onclick="document.getElementById('addLocationModal').classList.remove('hidden')">
      <!-- Heroicon: plus -->
  ...
  <!-- Add Location Modal (Tailwind, hidden by default) -->
  <div id="addLocationModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 hidden">
    <div class="bg-white rounded-lg shadow-lg w-full max-w-md mx-auto">
      <form method="post">
        <div class="flex items-center justify-between px-6 py-4 border-b">
          ...
          <button type="button" class="text-gray-400 hover:text-gray-700 text-xl font-bold" onclick="document.getElementById('addLocationModal').classList.add('hidden')">&times;</button>
        </div>
        <div class="px-6 py-4">
          <div class="mb-4">
            <label for="location_name" class="block text-sm font-medium text-gray-700 mb-1">Location Name</label>
            <input type="text" class="block w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" id="location_name" name="name" required>
          </div>
        </div>
        <div class="flex justify-end gap-2 px-6 py-4 border-t bg-gray-50 rounded-b-lg">
          <button type="button" class="px-4 py-2 rounded bg-gray-200 text-gray-700 hover:bg-gray-300" onclick="document.getElementById('addLocationModal').classList.add('hidden')">Cancel</button>
        ...
        <?php foreach ($rows as $r): ?>
          <tr>
            ...
            <td class="px-6 py-4 whitespace-nowrap flex gap-2">
              <a href="locations.php?id=<?=$r['id']?>" class="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 rounded hover:bg-blue-200 text-xs font-medium transition">Edit</a>
              <a href="locations.php?delete=<?=$r['id']?>" class="inline-flex items-center gap-1 px-3 py-1 bg-red-100 text-red-800 rounded hover:bg-red-200 text-xs font-medium transition" onclick="return confirm('Delete this location?')">Delete</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
...
<script>
  ...
  <?php if ($id): ?>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('location_name').value = <?php echo json_encode($name); ?>;
    });
  <?php endif; ?>
</script>
